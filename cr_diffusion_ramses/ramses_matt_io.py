...
def rd_amr(path, nout,**kwargs):
    ...
    for icpu in cpulist:
        ...
        car1 = str(nout).zfill(5)
        car2 = str(icpu).zfill(5)
        filename = path + "output_"+car1+"/amr_"+car1+".out"+car2
...
def rd_hydro(path, nout,**kwargs):
    car1 = str(nout).zfill(5)
    filename = path + "output_"+car1+"/hydro_"+car1+".out00001"
    with FortranFile(filename, 'r') as f:
        ncpu, = f.read_ints('i')
        nvar, = f.read_ints('i')
        ndim, = f.read_ints('i')
    ...
    for icpu in tqdm(cpulist):
        ...
        car1 = str(nout).zfill(5)
        car2 = str(icpu).zfill(5)
        filename = path + "output_"+car1+"/hydro_"+car1+".out"+car2
...
def rd_cell(path, nout,**kwargs):
    ...
    a = rd_amr(path, nout,**kwargs)
    h = rd_hydro(path, nout,**kwargs)
